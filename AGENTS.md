...
### 2.1 Подтверждение глубоких изменений

Если запрос затрагивает фундамент, основные технические правила, архитектурные границы или существенно меняет scope, сначала проверить, можно ли мягко применить его без противоречия действующим документам.

Если мягкое применение невозможно, кодер обязан до любых изменений:

1. явно назвать конфликтующие правила и последствия;
2. задать пользователю вопрос: «Вы просите глубокие изменения, которые противоречат основным правилам проекта. Если вы согласны изменить основные правила, ответьте “да”.»;
3. остановиться до точного утвердительного ответа `да`.

Молчание, отсутствие ответа, новая неоднозначная инструкция или продолжение обсуждения не считаются согласием. Без ответа `да` запрещено менять фундамент, технические спецификации и код в рамках конфликтующего решения.
...
- глубокое изменение не получило отдельного ответа `да` по правилу 2.1;
