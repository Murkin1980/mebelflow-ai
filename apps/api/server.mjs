...
import {
  createApiHandler,
  createFetchSender,
  createTurnstileWorkerVerifier,
  loadApiEnvironment,
} from "../../dist/packages/api-gateway/src/index.js";
...
} from "../../dist/packages/transcription-gateway/src/index.js";
...
if (!turnstileVerifyUrl || !turnstileExpectedHostname)
  throw new Error("TURNSTILE_VERIFY_URL and TURNSTILE_EXPECTED_HOSTNAME are required.");

const provider = new OpenAiIntentProvider({
  apiKey: env.OPENAI_API_KEY,
  model: env.OPENAI_MODEL,
  sender: createFetchSender(),
});
...
const humanVerifier = createTurnstileWorkerVerifier({
  url: turnstileVerifyUrl,
  expectedHostname: turnstileExpectedHostname,
});
const handler = createApiHandler({
  gateway,
  requestGate: coordination,
  humanVerifier,
  allowedOriginsByTenant: { [allowedTenant]: [allowedOrigin] },
});
const transcriptionHandler = createTranscriptionHandler({
  ...
  requestGate: coordination,
  store: coordination,
  provider: createOpenAiTranscriptionProvider({ apiKey: env.OPENAI_API_KEY }),
  tenantBudgetKzt: Number(process.env.TENANT_BUDGET_KZT ?? 50_000),
  usdKzt: env.USD_KZT_RATE,
});
...
const voiceTranscriptionHandler = createTranscriptionHandler({
  allowedOriginsByTenant: { [allowedTenant]: [allowedOrigin] },
  humanVerifier: preverifiedHuman,
  requestGate: coordination,
  store: coordination,
  provider: createOpenAiTranscriptionProvider({ apiKey: env.OPENAI_API_KEY }),
  tenantBudgetKzt: Number(process.env.TENANT_BUDGET_KZT ?? 50_000),
...
const voiceIntentHandler = createApiHandler({
  gateway,
  requestGate: coordination,
  humanVerifier: preverifiedHuman,
  allowedOriginsByTenant: { [allowedTenant]: [allowedOrigin] },
});
const voiceCommandHandler = createVoiceCommandHandler({
  allowedOriginsByTenant: { [allowedTenant]: [allowedOrigin] },
  humanVerifier,
  transcriptionHandler: voiceTranscriptionHandler,
  intentHandler: voiceIntentHandler,
});
...
const server = createServer(async (request, response) => {
  try {
    ...
    response.end(
      JSON.stringify({
        error: { code: tooLarge ? "PAYLOAD_TOO_LARGE" : "INVALID_JSON", message: "Запрос не обработан." },
      }),
    );
  }
});
