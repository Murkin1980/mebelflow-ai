...
describe("pilot configuration", () => {
  it("creates an allowlisted landing embed and rejects foreign origins", () => {
    expect(createEmbedSnippet(PILOT_TENANT_CONFIG, "https://grand-mebel.kz")).toContain(
      'data-tenant="grand-mebel-pilot"',
    );
...
describe("pilot telemetry", () => {
  it("records idempotent analytics with sanitized error codes only", () => {
    const events = new PilotEventCollector();
    const base = {
      id: "e1",
      tenantId: PILOT_TENANT_CONFIG.tenantId,
      sessionId: "s1",
    ...
    events.record(base);
    events.record(base);
    expect(events.list(PILOT_TENANT_CONFIG.tenantId)).toHaveLength(1);
  ...
  it("builds a cost dashboard and hard-stops at tenant budget", () => {
    const costs = new PilotCostCollector();
    costs.record({
      id: "c1",
      tenantId: PILOT_TENANT_CONFIG.tenantId,
      sessionId: "s1",
    ...
    expect(costs.dashboard(PILOT_TENANT_CONFIG.tenantId, 50_000)).toMatchObject({
      totalKzt: 50_000,
      remainingKzt: 0,
      hardStop: true,
    });
...
describe("pilot gate", () => {
  it("refuses to invent a go/no-go decision before real sample targets", () => {
    const report = createFunnelReport(
      PILOT_TENANT_CONFIG,
      new PilotStudy(),
      new PilotEventCollector(),
      new PilotCostCollector(),
    );
  ...
  it("accepts interview notes only for real prospects", () => {
    const study = new PilotStudy();
    study.addSession({
      id: "s1",
      tenantId: PILOT_TENANT_CONFIG.tenantId,
      kind: "controlled",
      participantCode: "TEST-01",
    ...
    expect(() =>
      study.addInterview({
        sessionId: "s1",
        participantCode: "TEST-01",
        note: "Пользователю было понятно управление.",
  ...
  it("rejects PII in interview notes and ignores events outside registered sessions", () => {
    ...
    expect(() =>
      study.addInterview({
        sessionId: "real-1",
        participantCode: "REAL-01",
        note: "Позвонить +7 701 123 45 67 после обеда",
        at: "2026-08-02T11:00:00Z",
      }),
    ).toThrow("телефон");
    const events = new PilotEventCollector();
  ...
  it("produces GO only after thresholds and healthy funnel metrics", () => {
    ...
    for (let index = 1; index <= 25; index++) {
      ...
      const prospect = index > 20;
      const participantCode = prospect ? `REAL-${index}` : `TEST-${index}`;
      study.addSession({
        id: sessionId,
        tenantId: PILOT_TENANT_CONFIG.tenantId,
        kind: prospect ? "real_prospect" : "controlled",
        participantCode,
        startedAt: "2026-08-02T10:00:00Z",
        completedAt: "2026-08-02T10:10:00Z",
      });
      if (prospect)
        study.addInterview({
          sessionId,
          participantCode,
          note: "Реальный участник понял сценарий и предварительную цену.",
