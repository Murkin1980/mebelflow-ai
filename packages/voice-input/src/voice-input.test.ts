...
const intent = (responses: ConstructorParameters<typeof FakeIntentProvider>[0]) =>
  new IntentSession(new FakeIntentProvider(responses));
const response = { transcript: "стена три метра", confidence: 0.96, billedSeconds: 12 };
const command = {
  output: {
    commandId: "wall-voice",
    type: "SET_WALL_WIDTH",
    payload: { width: 3000 },
    confidence: 0.98,
...
describe("VoiceSession", () => {
  ...
  it("shows transcript before confirmation and applies only after confirm", async () => {
    const session = new VoiceSession(
      new FakeSttProvider([response]),
      intent([command]),
      history(),
      new VoiceCostLedger(30),
    );
    await session.requestPermission(async () => "granted");
    session.startRecording();
  ...
  it("allows transcript correction before interpretation", async () => {
    ...
    const session = new VoiceSession(
      new FakeSttProvider([{ ...response, transcript: "стена триста" }]),
      new IntentSession(provider),
      history(),
    );
    await session.requestPermission(async () => "granted");
    session.startRecording();
  ...
  it("rejects empty and malformed STT output without cost or state changes", async () => {
    ...
    const session = new VoiceSession(
      new FakeSttProvider([{ transcript: "", confidence: 2, billedSeconds: -1 }]),
      intent([]),
      history(),
      costs,
    );
