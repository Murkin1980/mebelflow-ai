...
export class OpenAiIntentProvider implements IntentProvider {
  ...
  async complete(request: ProviderRequest): Promise<ProviderResponse> {
    const outputContract = [
      ...
      "MOVE_LEFT and MOVE_RIGHT require payload {id:string} and move the named existing module by exactly one neighboring position. MOVE_TO_EDGE requires payload {id:string,edge:'left'|'right'} and moves it to the far edge.",
      "For every move, resolve the customer's module name through projectSummary.modules and copy that existing module id exactly into payload.id. For example, 'перемести мойку вправо' means MOVE_RIGHT for the module whose type is sink_cabinet; 'мойку в правый край' means MOVE_TO_EDGE with edge right.",
      "Use MOVE_LEFT or MOVE_RIGHT for Russian 'влево' or 'вправо'. Use MOVE_TO_EDGE only for explicit edge phrases such as 'в левый край', 'в правый край', 'самой левой' or 'самой правой'.",
    ...
    const response = await this.sender({
      ...
      body: {
        ...
        input: [
          {
            role: "system",
            content: `${request.prompt.system} Allowed commands: ${request.prompt.allowedCommands.join(", ")}. ${outputContract}`,
          },
          {
            role: "user",
              ...
              properties: {
                command: {
                  ...
                  properties: {
                    ...
                    type: { type: "string", enum: request.prompt.allowedCommands },
                    payload: { type: "object" },
...
export const GPT5_MINI_PRICE = {
  version: GPT5_MINI_PRICE_VERSION,
...
export function calculateGpt5MiniCost(usage: ProviderUsage, usdKzt: number) {
  ...
  const costUsd =
    ((usage.inputTokens - cached) * GPT5_MINI_PRICE.inputUsdPerMillion +
      cached * GPT5_MINI_PRICE.cachedInputUsdPerMillion +
      usage.outputTokens * GPT5_MINI_PRICE.outputUsdPerMillion) /
    1_000_000;
  return {
    model: GPT5_MINI_PRICE.model,
    priceVersion: GPT5_MINI_PRICE.version,
    usdKzt,
    costUsd: Math.round(costUsd * 100_000_000) / 100_000_000,
    costKzt: Math.round(costUsd * usdKzt * 100) / 100,
  };
}
...
export class MemoryGatewayStore implements GatewayStore {
  ...
  async acquireSession(tenantId: string, sessionId: string, _ttlSeconds: number) {
  ...
  async releaseSession(tenantId: string, sessionId: string, leaseToken: string) {
    const key = this.key(tenantId, sessionId);
    if (this.locks.get(key) === leaseToken) this.locks.delete(key);
  }
  async getSessionUsage(tenantId: string, sessionId: string) {
    return structuredClone(
      this.sessions.get(this.key(tenantId, sessionId)) ?? { calls: 0, inputTokens: 0, outputTokens: 0, costKzt: 0 },
    );
  }
  async getTenantSpentKzt(tenantId: string) {
    return this.tenants.get(tenantId) ?? 0;
  }
  async reserveTenantBudget(tenantId: string, reservationId: string, amountKzt: number, budgetKzt: number) {
    ...
    if ((this.tenants.get(tenantId) ?? 0) + (this.reserved.get(tenantId) ?? 0) + amountKzt > budgetKzt) return false;
    this.reservations.add(key);
    this.reserved.set(tenantId, money((this.reserved.get(tenantId) ?? 0) + amountKzt));
    return true;
  }
  async cancelTenantBudget(tenantId: string, reservationId: string, amountKzt: number) {
    const key = `${tenantId}:${reservationId}`;
    if (!this.reservations.delete(key)) return;
    this.reserved.set(tenantId, Math.max(0, money((this.reserved.get(tenantId) ?? 0) - amountKzt)));
  }
  async commitUsage(input: UsageCommit) {
    const reservationKey = `${input.tenantId}:${input.reservationId}`;
    if (!this.reservations.delete(reservationKey)) throw new Error("BUDGET_RESERVATION_MISSING");
    this.reserved.set(input.tenantId, Math.max(0, money((this.reserved.get(input.tenantId) ?? 0) - input.reservedKzt)));
    const key = this.key(input.tenantId, input.sessionId);
    const current = await this.getSessionUsage(input.tenantId, input.sessionId);
    const next = {
      calls: current.calls + 1,
      inputTokens: current.inputTokens + input.usage.inputTokens,
      outputTokens: current.outputTokens + input.usage.outputTokens,
      costKzt: Math.round((current.costKzt + input.costKzt) * 100) / 100,
    };
    this.sessions.set(key, next);
    this.tenants.set(input.tenantId, Math.round(((this.tenants.get(input.tenantId) ?? 0) + input.costKzt) * 100) / 100);
...
export class AiGateway {
  ...
  constructor(
    ...
    private readonly store: GatewayStore = new MemoryGatewayStore(),
  ) {
  ...
  async warmup() {
    ...
    this.warmed = true;
    return { status: "ready" as const, billableAiCalls: 0 };
  }
  isWarm() {
    return this.warmed;
  }
  async complete(sessionId: string, request: ProviderRequest) {
    return this.completeForTenant("default", sessionId, request);
  }
  async completeForTenant(tenantId: string, sessionId: string, request: ProviderRequest) {
    ...
    const reservedKzt = money(
      ((this.limits.maxInputTokensPerSession * GPT5_MINI_PRICE.inputUsdPerMillion +
        this.limits.maxOutputTokensPerSession * GPT5_MINI_PRICE.outputUsdPerMillion) /
        1_000_000) *
        this.limits.usdKzt,
    );
    ...
    try {
      ...
      if (
        current.inputTokens >= this.limits.maxInputTokensPerSession ||
        current.outputTokens >= this.limits.maxOutputTokensPerSession
      )
        throw new Error("SESSION_TOKEN_LIMIT");
      if (!(await this.store.reserveTenantBudget(tenantId, reservationId, reservedKzt, this.limits.tenantBudgetKzt)))
        throw new Error("TENANT_BUDGET_EXCEEDED");
      budgetReserved = true;
      const response = await this.provider.complete(request);
      const priced = calculateGpt5MiniCost(response.usage, this.limits.usdKzt);
      const next = {
        calls: current.calls + 1,
        inputTokens: current.inputTokens + response.usage.inputTokens,
        outputTokens: current.outputTokens + response.usage.outputTokens,
        costKzt: Math.round((current.costKzt + priced.costKzt) * 100) / 100,
      };
      if (
        next.inputTokens > this.limits.maxInputTokensPerSession ||
        next.outputTokens > this.limits.maxOutputTokensPerSession
      )
        throw new Error("SESSION_TOKEN_LIMIT");
      const committed = await this.store.commitUsage({
        tenantId,
        sessionId,
        usage: response.usage,
        costKzt: priced.costKzt,
        reservationId,
        reservedKzt,
      });
      ...
      if (budgetReserved) await this.store.cancelTenantBudget(tenantId, reservationId, reservedKzt);
      this.activeRequests -= 1;
      await this.store.releaseSession(tenantId, sessionId, leaseToken);
    }
  }
  usage(sessionId: string) {
    return this.store.getSessionUsage("default", sessionId);
  }
  async tenantUsage(tenantId = "default") {
    const spentKzt = await this.store.getTenantSpentKzt(tenantId);
    return {
      spentKzt,
      budgetKzt: this.limits.tenantBudgetKzt,
      remainingKzt: Math.max(0, this.limits.tenantBudgetKzt - spentKzt),
    };
  }
}
