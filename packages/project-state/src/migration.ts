import { createInitialProject,FurnitureProjectStateSchema,type FurnitureProjectState } from "./schema.js";
...
export function recoverProjectState(input:unknown,projectId:string,tenantId:string):RecoveryResult{
  try{return {state:migrateProjectState(input),recovered:false}}catch(error){
    const state=createInitialProject(projectId,tenantId);
    state.warnings.push({code:"state_recovered",message:"Повреждённое состояние не применено. Создан безопасный новый проект."});
