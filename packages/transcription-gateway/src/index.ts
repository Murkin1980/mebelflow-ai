...
export type TranscriptionProvider = (input: {
  audio: Uint8Array;
  mimeType: string;
...
const error = (status: number, code: string, message: string): TranscriptionApiResponse => ({
  status,
...
export function calculateTranscriptionCost(usage: TranscriptionUsage, usdKzt: number) {
  const costUsd =
    (usage.inputTokens * GPT4O_MINI_TRANSCRIBE_PRICE.inputUsdPerMillion +
      usage.outputTokens * GPT4O_MINI_TRANSCRIBE_PRICE.outputUsdPerMillion) /
    1_000_000;
...
export function createOpenAiTranscriptionProvider(config: {
  ...
  return async (input) => {
    ...
    form.append(
      "prompt",
      "Кухня, мойка, посудомоечная машина, ПММ, варочная панель, духовой шкаф, фасад, столешница, миллиметры.",
    );
...
export function createTranscriptionHandler(config: {
  ...
  const reserved = calculateTranscriptionCost(
    { inputTokens: config.reservationInputTokens ?? 100_000, outputTokens: config.reservationOutputTokens ?? 20_000 },
    config.usdKzt,
  ).costKzt;
  return async (request: { origin?: string; body: unknown }): Promise<TranscriptionApiResponse> => {
    ...
    if (!request.origin || !(config.allowedOriginsByTenant[input.tenantId] ?? []).includes(request.origin))
      return error(403, "ORIGIN_NOT_ALLOWED", "Этот сайт не подключён к MebelFlow.");
    let verified: boolean;
    try {
      verified = await config.humanVerifier.verify({
        token: input.turnstileToken,
        idempotencyKey: input.idempotencyKey,
      });
    } catch {
      return error(503, "TURNSTILE_VERIFY_UNAVAILABLE", "Проверка безопасности временно недоступна.");
    }
    ...
    const gate = await config.requestGate.consume({
      ...
      sessionId: `stt_${input.sessionId}`,
      idempotencyKey: input.idempotencyKey,
      atMs: now(),
    });
    if (gate === "duplicate") return error(409, "DUPLICATE_REQUEST", "Голосовая команда уже была отправлена.");
    if (gate === "rate_limited")
      return error(429, "RATE_LIMITED", "Слишком много запросов. Подождите несколько секунд.");
    const audio = Uint8Array.from(Buffer.from(input.audioBase64, "base64"));
    if (!audio.byteLength || audio.byteLength > 1_000_000)
      return error(400, "INVALID_AUDIO", "Запись пуста или превышает допустимый размер.");
    const reservationId = crypto.randomUUID();
    if (!(await config.store.reserveTenantBudget(input.tenantId, reservationId, reserved, config.tenantBudgetKzt)))
      return error(429, "TENANT_BUDGET_EXCEEDED", "Лимит голосовых запросов исчерпан.");
    ...
    try {
      ...
      await config.store.commitUsage({
        tenantId: input.tenantId,
        sessionId: `stt_${input.sessionId}`,
        usage: result.usage,
        costKzt: cost.costKzt,
        reservationId,
        reservedKzt: reserved,
      });
      ...
      return error(
        ...
        "Не удалось расшифровать запись. Повторите отправку или используйте текст.",
      );
    } finally {
...
export function createVoiceCommandHandler(config: {
  ...
  return async (request: { origin?: string; body: unknown }): Promise<TranscriptionApiResponse> => {
    ...
    const transcription = await config.transcriptionHandler({
      origin: request.origin,
      body: {
        tenantId: input.tenantId,
        sessionId: input.sessionId,
        idempotencyKey: `${input.idempotencyKey}_stt`,
        turnstileToken: "server-verified",
    ...
    const intent = await config.intentHandler({
      method: "POST",
      path: "/v1/intent",
      origin: request.origin,
      body: {
        tenantId: input.tenantId,
        sessionId: input.sessionId,
        idempotencyKey: `${input.idempotencyKey}_intent`,
        turnstileToken: "server-verified",
        locale: input.locale,
