...
class FakeRef { constructor(readonly path: string, private readonly db: FakeFirestore) {} get() { return Promise.resolve(this.db.snapshot(this)); } }
class FakeFirestore {
  readonly data = new Map<string, Record<string, unknown>>();
  collection(name: string) { return { doc: (key: string) => new FakeRef(`${name}/${key}`, this) }; }
  snapshot(ref: FakeRef) { const value = this.data.get(ref.path); return { exists: Boolean(value), data: () => value ? structuredClone(value) : undefined }; }
  async runTransaction<T>(work: (tx: any) => Promise<T>) {
    const writes: (() => void)[] = [];
    const tx = {
      get: (ref: FakeRef) => Promise.resolve(this.snapshot(ref)),
      set: (ref: FakeRef, value: Record<string, unknown>, options?: { merge?: boolean }) => writes.push(() => this.data.set(ref.path, options?.merge ? { ...this.data.get(ref.path), ...value } : structuredClone(value))),
      create: (ref: FakeRef, value: Record<string, unknown>) => writes.push(() => { if (this.data.has(ref.path)) throw new Error("already exists"); this.data.set(ref.path, structuredClone(value)); }),
      delete: (ref: FakeRef) => writes.push(() => { this.data.delete(ref.path); }),
    };
...
describe("FirestoreCoordinationStore", () => {
  it("uses expiring lease tokens and ignores stale release", async () => {
    const db = new FakeFirestore(); let now = 1_000; let token = 0;
    const store = new FirestoreCoordinationStore(db as unknown as Firestore, 2, () => now, () => `lease-${++token}`);
  ...
  it("persists usage atomically in session and tenant ledgers", async () => {
    ...
    await store.commitUsage({ tenantId: "tenant", sessionId: "session", usage: { inputTokens: 10, outputTokens: 3 }, costKzt: 0.51, reservationId: "r1", reservedKzt: 2 });
    await store.reserveTenantBudget("tenant", "r2", 2, 10);
    const usage = await store.commitUsage({ tenantId: "tenant", sessionId: "session", usage: { inputTokens: 5, outputTokens: 2 }, costKzt: 0.52, reservationId: "r2", reservedKzt: 2 });
  ...
  it("reclaims an abandoned reservation after its lease expires", async () => {
    let now = 1_000; const store = new FirestoreCoordinationStore(new FakeFirestore() as unknown as Firestore, 2, () => now);
    expect(await store.reserveTenantBudget("tenant", "r1", 8, 10)).toBe(true);
    now += 120_001;
    expect(await store.reserveTenantBudget("tenant", "r2", 8, 10)).toBe(true);
  });
