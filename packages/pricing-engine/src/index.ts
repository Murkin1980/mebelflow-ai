...
const CommonSchema = z.object({
  ...
  uncertaintyPercent: z.number().min(0).max(0.5),
  upperRowRatePerMeter: z.number().nonnegative(),
  facadeRatePerMeter: z.number().nonnegative(),
  hardwarePerModule: z.number().nonnegative(),
});
const PerMeterSchema = CommonSchema.extend({
  strategy: z.literal("per_meter"),
  baseRatePerMeter: z.number().positive(),
});
const PerModuleSchema = CommonSchema.extend({
  strategy: z.literal("per_module"),
  defaultModulePrice: z.number().positive(),
  modulePrices: z.record(z.string(), z.number().positive()).default({}),
});
const HybridSchema = CommonSchema.extend({
  strategy: z.literal("hybrid"),
  baseRatePerMeter: z.number().positive(),
  defaultModuleSurcharge: z.number().nonnegative(),
  moduleSurcharges: z.record(z.string(), z.number().nonnegative()).default({}),
});
...
export type PriceBreakdown = {
  code: "base_modules" | "upper_modules" | "facades" | "hardware";
  min: number;
  max: number;
};
...
function ranged(code: PriceBreakdown["code"], nominal: number, uncertainty: number): PriceBreakdown {
  return {
    code,
    min: Math.max(0, roundKzt(nominal * (1 - uncertainty))),
    max: Math.max(0, roundKzt(nominal * (1 + uncertainty))),
  };
}
...
export function calculatePrice(state: FurnitureProjectState, input: PricingConfig): PriceEstimate {
  ...
  else if (config.strategy === "per_module")
    baseNominal = modules.reduce(
      (sum, module) => sum + (config.modulePrices[module.type] ?? config.defaultModulePrice),
      0,
    );
  else
    baseNominal =
      meters * config.baseRatePerMeter +
      modules.reduce((sum, module) => sum + (config.moduleSurcharges[module.type] ?? config.defaultModuleSurcharge), 0);
  ...
  return {
    ...
    breakdown,
    reasons: [
      "Точные материалы и фурнитура уточняются после замера.",
      ...(state.style.presetId ? [] : ["Стиль ещё не выбран; применён базовый коэффициент фасада."]),
    ],
    disclaimer: "Предварительная стоимость. Окончательная цена подтверждается мебельщиком после проверки и замера.",
  };
}
