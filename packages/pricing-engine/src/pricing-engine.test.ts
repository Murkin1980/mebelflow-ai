...
import { applyPriceEstimate, calculatePrice, type PricingConfig, pricePresentation } from "./index.js";
...
const common = {
  ...
  upperRowRatePerMeter: 70_000,
  facadeRatePerMeter: 50_000,
  hardwarePerModule: 15_000,
};
...
describe("pricing strategies", () => {
  it.each<PricingConfig>([
    ...
    { ...common, strategy: "per_module", defaultModulePrice: 160_000, modulePrices: { dishwasher_600: 120_000 } },
    {
      ...common,
      strategy: "hybrid",
      baseRatePerMeter: 120_000,
      defaultModuleSurcharge: 20_000,
      moduleSurcharges: { sink_cabinet: 40_000 },
    },
  ...
  it("recalculates when width, modules, upper row or facade changes", () => {
    ...
    const wider = project();
    wider.room.wallWidth = 4000;
    const second = calculatePrice(wider, config);
    const styled = applyStyleSelection(wider, { styleId: "japandi", paletteId: "sage", facadeId: "wood_veneer" }, [
      "japandi",
    ]);
    const third = calculatePrice(styled, config);
    expect(second.max).toBeGreaterThan(first.max);
    expect(third.breakdown.find((item) => item.code === "facades")!.max).toBeGreaterThan(
      second.breakdown.find((item) => item.code === "facades")!.max,
    );
  });

  it("excludes fillers from module and hardware pricing", () => {
    const state = project();
    state.lowerRow.modules.push({ id: "fill", type: "filler", width: 50, height: 720, depth: 560, position: 2000 });
  ...
  it("requires wall width and validates tenant configuration", () => {
    const state = project();
    state.room.wallWidth = null;
    expect(() => calculatePrice(state, { ...common, strategy: "per_meter", baseRatePerMeter: 100_000 })).toThrow(
      "ширину стены",
    );
    expect(() =>
      calculatePrice(project(), {
        ...common,
        uncertaintyPercent: 0.8,
        strategy: "per_meter",
        baseRatePerMeter: 100_000,
      }),
    ).toThrow();
  });
  ...
  it("stores a preliminary estimate without mutating the source state", () => {
    ...
    expect(next).toMatchObject({
      stage: "price_shown",
      pricing: { estimateMin: estimate.min, estimateMax: estimate.max, formulaVersion: 3, status: "preliminary" },
    });
