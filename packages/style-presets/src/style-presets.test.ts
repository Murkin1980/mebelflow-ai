...
import { applyStyleSelection, FACADE_PRESETS, getTenantStyles, STYLE_PRESETS, selectedFacade } from "./index.js";
...
describe("style presets", () => {
  ...
  it("applies an allowed style selection and advances project stage", () => {
    const state = applyStyleSelection(
      createInitialProject("p", "t"),
      { styleId: "japandi", paletteId: "sage", facadeId: "wood_veneer" },
      ["japandi"],
    );
  ...
  it("rejects tenant-disabled styles and mismatched palettes/facades", () => {
    ...
    expect(() =>
      applyStyleSelection(state, { styleId: "japandi", paletteId: "sage", facadeId: "wood_veneer" }, ["scandinavian"]),
    ).toThrow("недоступен");
    expect(() =>
      applyStyleSelection(state, { styleId: "japandi", paletteId: "snow", facadeId: "wood_veneer" }, ["japandi"]),
    ).toThrow("Палитра");
    expect(() =>
      applyStyleSelection(state, { styleId: "japandi", paletteId: "sage", facadeId: "shaker" }, ["japandi"]),
    ).toThrow("Фасад");
  });
});
