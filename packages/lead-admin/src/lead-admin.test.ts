...
import {
  FakeContactVerificationProvider,
  FakeLeadNotifier,
  FakeResumeTokenProvider,
...
const project = () => {
  const state = createInitialProject("project-1", "tenant-1");
...
const settings = TenantSettingsSchema.parse({
  tenantId: "tenant-1",
  brandName: "Grand Mebel",
  locale: "ru-KZ",
  phone: "+77011234567",
  whatsapp: null,
  telegramChatId: "chat",
  enabledStyleIds: ["japandi"],
  standardWidths: [300, 450, 600],
  limits: { commandsPerSession: 2, sttSecondsPerSession: 30, projectsPerMonth: 5, aiBudgetKztPerMonth: 100 },
  disclaimer: "Предварительная цена требует проверки и замера.",
  retentionDays: 30,
});
...
describe("LeadService", () => {
  ...
  it("is idempotent by request id and does not notify twice", async () => {
    const notifier = new FakeLeadNotifier();
    const service = new LeadService(new InMemoryLeadRepository(), new FakeContactVerificationProvider(), notifier);
  ...
  it("enforces OTP and CAPTCHA after threshold", async () => {
    await expect(
      new LeadService(
        ...
        new FakeContactVerificationProvider(false),
        new FakeLeadNotifier(),
      ).create(input),
    ).rejects.toThrow("код");
    const verification = new FakeContactVerificationProvider(true, false);
    await expect(
      new LeadService(new InMemoryLeadRepository(), verification, new FakeLeadNotifier()).create({
        ...input,
        attemptNumber: 3,
        captchaToken: "bad",
      }),
  ...
  it("validates contact consent and Kazakhstan phone format", async () => {
    const service = new LeadService(
      new InMemoryLeadRepository(),
      new FakeContactVerificationProvider(),
      new FakeLeadNotifier(),
    );
    ...
    await expect(service.create({ ...input, requestId: "request-attempt", attemptNumber: 0 })).rejects.toThrow(
      "попытки",
    );
  });
  ...
  it("keeps a lead when Telegram notification fails and records the failure", async () => {
    const service = new LeadService(
      new InMemoryLeadRepository(),
      new FakeContactVerificationProvider(),
      new FakeLeadNotifier(new Error("offline")),
    );
    const lead = await service.create(input);
  ...
  it("enforces status transitions and keeps append-only audit events", async () => {
    ...
    expect(service.details("lead-1").events.map((event) => event.type)).toEqual([
      "created",
      "notification_sent",
      "status_changed",
      "status_changed",
...
describe("tenant controls", () => {
  it("hard-stops usage at tenant limits", () => {
    const guard = new UsageGuard(settings);
    guard.consume("commandsPerSession", 2);
  ...
  it("restores only an opaque token from the expected tenant", async () => {
    const provider = new FakeResumeTokenProvider();
    const token = await provider.issue("tenant-1", "project-1");
    await expect(resumeProject(provider, token, "other-tenant", async () => project())).rejects.toThrow(
      "недействительна",
    );
    await expect(
      resumeProject(provider, token, "tenant-1", async () => ({ ...project(), projectId: "wrong" })),
